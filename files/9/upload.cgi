...
def check_auth(username, branchname, groupmemberships=None):
    ...
    for group in groupmemberships:
        ...
        if re.match(r'c\d+s?-{0}.*'.format(group), branchname):
            stderr_message = 'Matched %s against %s'  % (group, branchname)
            sys.stderr.write(str(stderr_message))
            return True
        else:
            sys.stderr.write('No matching group returned')
...
def main():
    ...
    sys.stderr.write(str(module_dir))
    dest_file = os.path.join(module_dir, checksum)
    sys.stderr.write(str(dest_file))
    ...
    if conf.getboolean('acls', 'do_acl'):
        if not check_auth(username, branch):
            ...
            sys.stderr.write('Issue verifying acl \n')
            sys.exit(0)
        else:
            sys.stderr.write('ACL validated \n')
    ...
    if not os.path.isdir(module_dir):
        try:
            ...
            print('Content-type: text/plain')
            print()
            sys.stderr.write('Issue Creating directory \n')
            sys.exit(0)
    ...
    stderr_message = '[username=%s] Stored %s (%s bytes)' % (username, dest_file, filesize)
    sys.stderr.write(str(stderr_message))
    print ('Content-Type: text/plain')
    print ()
    status_message = 'File %s stored OK (checksum %s , size %s)' % (filename, checksum, filesize)
